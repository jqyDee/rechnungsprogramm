...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.sidebar_button_rechnung_loeschen = customtkinter.CTkButton(self.sidebar_frame,
                                                                        text="Rechnung Loeschen",
    ...
    def kuerzel_neue_rechnung_keystroke_check_event(self, t):
        ...
        if os.path.exists("./Stammdaten/" + entry + ".txt"):
            ...
            logging.error("'stammdatei' doesn't exist")
            clear_rechnung_screen()
            return False
    ...
    def behandlungsarten_keystroke_check_event(self, t):
        ...
        try:
            ...
            if entry == "":
                logging.error("must not '' -> clearing")
                self.info_message("neue_rechnung", "Error: must not '' -> clearing", "red")
                ...
                logging.error("only integer -> clearing")
    ...
    def neue_rechnung_interface(self):
        logging.info("configuring 'neue rechnungen' interface")
        # Heutiges Datum
    ...
    def neue_rechnung_abschicken_button_event(self):
        ...
        self.rechnung_error_msg.grid_forget()
        ...
        logging.debug("Rechnungsnummer: " + str(rechnungsnummer))

        # check Rechnung existance
        if os.path.exists("./Rechnungen/Rechnungen{0}/".format(self.version) + rechnungsnummer + ".pdf"):
            self.info_message("neue_rechnung", "Error: 'Rechnung: {0}' already exists".format(rechnungsnummer), "red",
                              True)
            return
        ...
        try:
            ...
            logging.info("couldn't convert 'anzahl behandlungsdaten' into integer")
    ...
    def pdf_erstellen(self, rechnungsnummer, datum, stammdaten, daten_liste, datenzahl,
        ...
        self.info_message("neue_rechnung", "Saved successfully!", "green", True)
        logging.info("PDF successfully created")

        # PDF Öffnen
        subprocess.Popen(os.getcwd()+"/Rechnungen/Rechnungen{0}/".format(self.version) + rechnungsnummer + ".pdf", shell=True)
        logging.info("PDF opened")
    ...
    def stammdaten_abschicken_button_event(self):
        logging.info("saving stammdaten")
    ...
    # main events
    def info_message(self, frame, error_msg, color, additional_arg=None):
        ...
        if frame == "neue_rechnung":
            self.behandlungsarten_error_1.grid_forget()
            self.rechnung_error_msg.grid_forget()
            if additional_arg is True:
                self.rechnung_error_msg.configure(text=error_msg, text_color=color)
                self.rechnung_error_msg.grid(row=0, column=1, padx=5, pady=10)
            else:
            ...
            logging.debug("Error/Info: {0};{1};{2}".format(str(frame), str(error_msg), str(color)))
    ...
    def clear_screen_event(self):
        logging.info("Clear Screen Event")
        try:
            self.main_1.destroy()
            logging.debug("screen 1 cleared")
        except (AttributeError, tkinter.TclError):
            try:
                self.main_2.destroy()
                logging.debug("screen 2 cleared")
            except (AttributeError, tkinter.TclError):
                try:
                    self.main_3.destroy()
                    logging.debug("screen 3 cleared")
                except (AttributeError, tkinter.TclError):
                    try:
                        self.main_4.destroy()
                        logging.debug("screen 4 cleared")
                    except (AttributeError, tkinter.TclError):
